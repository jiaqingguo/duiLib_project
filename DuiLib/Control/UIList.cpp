...
namespace DuiLib {
	...
	void CListContainerElementUI::DoEvent(TEventUI& event)
	{
		...
		//{
		//	if (m_pManager != NULL) {
		//		m_pManager->SendNotify(this, DUI_MSGTYPE_ITEMRCLICK, event.wParam, event.lParam);//传递之后设定的消息事件
		//		Select(true);
		...
		if (m_pOwner != NULL)
		{
			if ( event.Type == UIEVENT_RBUTTONDOWN)//m_pOwner->GetListInfo()->bRSelected &&
			{
				if (IsEnabled()) {
