...
bool CChartViewUI::Add(map<CDuiString,double> SirenData)
{
	map<CDuiString, double>::iterator iter;
	CHARTITEM item;
	CDuiString nameAdd;
	m_items.clear();
	for (iter = SirenData.begin(); iter != SirenData.end(); iter++){
		
		nameAdd.Format(_T("%.0lf"),iter->second);

		item.name=iter->first;
		item.name+=_T("{c #FE5900}");
		item.name+=nameAdd;
		item.name+=_T(" {/c}");
